...
// VNADLL_EXPORTS should only be defined when building the DLL.
// Do not define it when linking to this DLL
#if defined(_MSC_VER)
	#ifdef VNADLL_EXPORTS
		#pragma message ("Building with dllexport")
		#define VNADLL_API extern __declspec(dllexport)
		#define xxxDLL_API extern __declspec(dllexport)
	#else
		#pragma message ("Building with dllimport")
		#define VNADLL_API extern __declspec(dllimport)
		#define xxxDLL_API extern __declspec(dllimport)
	#endif
#endif
#ifdef LINUX
	#ifdef xxxDLL_EXPORTS
		#define VNADLL_API extern __attribute__((visibility("default")))
		#define xxxDLL_API extern __attribute__((visibility("default")))
	#else
		#define VNADLL_API extern
		#define xxxDLL_API extern
	#endif
#endif
	 ...
	 */
	typedef int ErrCode;
	VNADLL_API ErrCode ERR_OK;                    //!< Return "error" code indicating no error occured.
	VNADLL_API ErrCode ERR_BAD_ATTEN;             //!< Error code indicating the specified attenuation value was not valid.
	VNADLL_API ErrCode ERR_BAD_CAL;               //!< Error code indicating the current calibration is invalid.
	VNADLL_API ErrCode ERR_BAD_HANDLE;            //!< Error code indicating the current handle passed to a DLL function was not valid.
	VNADLL_API ErrCode ERR_BAD_HOP;               //!< Error code indicating the specified hop-rate value was not valid.
	VNADLL_API ErrCode ERR_BAD_PATH;              //!< Error code indicating the specified path is not allowed, or was invalid.
	VNADLL_API ErrCode ERR_BAD_PROM;              //!< Error code indicating that the AVMU PROM was not in a known format.  Please contact support if this is a reoccuring issue
	VNADLL_API ErrCode ERR_BYTES;                 //!< Error code indicating the DLL has received an incorrect number of bytes from the AVMU. Possibly indicative of a network issue.
	VNADLL_API ErrCode ERR_EMPTY_PROM;            //!< Error code indicating the remote device's PROM appears to be empty.
	VNADLL_API ErrCode ERR_FEATURE_NOT_PRESENT;   //!< Error code indicating A THING
	VNADLL_API ErrCode ERR_FREQ_OUT_OF_BOUNDS;    //!< Error code indicating one of the specified frequency bounds was beyong the hardware's available range.
	VNADLL_API ErrCode ERR_INTERRUPTED;           //!< Error code indicating that the measurement was asynchronously interrupted from another thread.
	VNADLL_API ErrCode ERR_MISSING_FREQS;         //!< Error code indicating you have not yet specified the frequency endpoints for the remote device before connecting.
	VNADLL_API ErrCode ERR_MISSING_HOP;           //!< Error code indicating you have not yet specified the hop-rate for the remote device before connecting.
	VNADLL_API ErrCode ERR_MISSING_IP;            //!< Error code indicating you have not yet specified the IP of the remote device before connecting.
	VNADLL_API ErrCode ERR_MISSING_PORT;          //!< Error code indicating you have not yet specified the port for the remote device before connecting.
	VNADLL_API ErrCode ERR_NO_PATHS_MEASURED;     //!< Error code indicating there are no paths to measure, and the hardware has nothing to do.
	VNADLL_API ErrCode ERR_NO_RESPONSE;           //!< Error code indicating the remote instrument failed to respond to commands within the current timeout period.
	VNADLL_API ErrCode ERR_PATH_ALREADY_MEASURED; //!< Error code indicating you are adding a path to measure that you had already added.
	VNADLL_API ErrCode ERR_PROG_OVERFLOW;         //!< Error code indicating the generated program to run on the remote device was larger then it can handle.
	VNADLL_API ErrCode ERR_SOCKET;                //!< Error code indicating an unknown socket error occured.
	VNADLL_API ErrCode ERR_TOO_MANY_POINTS;       //!< Error code indicating you are trying to take more data points then the hardware is capable of.
	VNADLL_API ErrCode ERR_UNKNOWN_FEATURE;       //!< Error code indicating ANOTHER THING
	VNADLL_API ErrCode ERR_WRONG_PROGRAM_TYPE;    //!< Error code indicating you are in sync mode and are trying to call an async startup function.
	VNADLL_API ErrCode ERR_WRONG_STATE;           //!< Error code indicating the hardware is not in the correct state for the function you are trying to call.
	VNADLL_API ErrCode ERR_MISSING_ATTEN;         //!< THINGZ
	VNADLL_API ErrCode ERR_NO_ATTEN_PRESENT;      //!< THINGZ
	VNADLL_API ErrCode ERR_BAD_PORT;              //!< THINGZ
	...
	VNADLL_API HopRate HOP_UNDEFINED;  //!< Hop rate not set yet
	#ifdef VNADLL_API  // Only export for Avmus, the VNA doesn't support it.
		VNADLL_API HopRate HOP_90K;  //!< This rate is currently unsupported but may be enabled in the future 90K Pts/second
	#endif
	VNADLL_API HopRate HOP_45K;  //!< Hop rate of 45K points/second
	VNADLL_API HopRate HOP_30K;  //!< Hop rate of 30K points/second
	VNADLL_API HopRate HOP_15K;  //!< Hop rate of 15K points/second
	VNADLL_API HopRate HOP_7K;   //!< Hop rate of 7K points/second
	VNADLL_API HopRate HOP_3K;   //!< Hop rate of 3K points/second
	VNADLL_API HopRate HOP_2K;   //!< Hop rate of 2K points/second
	VNADLL_API HopRate HOP_1K;   //!< Hop rate of 1K points/second
	VNADLL_API HopRate HOP_550;  //!< Hop rate of 550 points/second
	VNADLL_API HopRate HOP_312;  //!< Hop rate of 312 points/second
	VNADLL_API HopRate HOP_156;  //!< Hop rate of 156 points/second
	VNADLL_API HopRate HOP_78;   //!< Hop rate of 78 points/second
	VNADLL_API HopRate HOP_39;   //!< Hop rate of 39 points/second
	VNADLL_API HopRate HOP_20;   //!< Hop rate of 20 points/second
	/** @}*/
	...
	VNADLL_API TaskState TASK_UNINITIALIZED; //!< Task state of uninitialized
	VNADLL_API TaskState TASK_STOPPED;       //!< Task state of stopped
	VNADLL_API TaskState TASK_STARTED;       //!< Task state of started
	VNADLL_API TaskState TASK_RUNNING;       //!< Task state of running
	...
	*/
	...
	VNADLL_API ProgramType PROG_ASYNC;  //!< Asynchronous acquisition mode
	VNADLL_API ProgramType PROG_SYNC;   //!< Synchronous acquisition mode
	...
	*/
	 ...
	 *       - TODO: More
	 */
	VNADLL_API ErrCode setShaftEncoderFeature(TaskHandle t, const bool enable, const bool reset_on_start);
	 ...
	 *       - TODO: More
	 */
	VNADLL_API ErrCode setSerialPortFeature(TaskHandle t, const bool enable, const unsigned int buffer_size);
	 ...
	 *       - TODO: More
	 */
	VNADLL_API ErrCode setMeasurementType(TaskHandle t, const ProgramType type);
	 ...
	 */
	VNADLL_API ProgramType getMeasurementType(TaskHandle t);
	...
	*/
	VNADLL_API ErrCode getFrequencies(TaskHandle t, double* freqs, int freqs_sz);
	 ...
	 *      - ERR_WRONG_STATE if the Task is not in the TASK_STOPPED state
	*/
	VNADLL_API ErrCode setSendSweepTimer(TaskHandle t, bool send_timer);
	VNADLL_API ErrCode getSendSweepTimer(TaskHandle t, bool* val);
	 ...
	 *      - ERR_WRONG_STATE if the Task is not in the TASK_STOPPED state
	*/
	VNADLL_API ErrCode setResetFrameCounterOnStart(TaskHandle t, bool do_reset);
	VNADLL_API ErrCode getResetFrameCounterOnStart(TaskHandle t, bool* val);
	...
	VNADLL_API Attenuation ATTEN_UNDEFINED;  //!< Attenuation value not set
	VNADLL_API Attenuation ATTEN_0;          //!< Attenuation value of 0 dB
	VNADLL_API Attenuation ATTEN_1;          //!< Attenuation value of 1 dB
	VNADLL_API Attenuation ATTEN_2;          //!< Attenuation value of 2 dB
	VNADLL_API Attenuation ATTEN_3;          //!< Attenuation value of 3 dB
	VNADLL_API Attenuation ATTEN_4;          //!< Attenuation value of 4 dB
	VNADLL_API Attenuation ATTEN_5;          //!< Attenuation value of 5 dB
	VNADLL_API Attenuation ATTEN_6;          //!< Attenuation value of 6 dB
	VNADLL_API Attenuation ATTEN_7;          //!< Attenuation value of 7 dB
	VNADLL_API Attenuation ATTEN_8;          //!< Attenuation value of 8 dB
	VNADLL_API Attenuation ATTEN_9;          //!< Attenuation value of 9 dB
	VNADLL_API Attenuation ATTEN_10;         //!< Attenuation value of 10 dB
	VNADLL_API Attenuation ATTEN_11;         //!< Attenuation value of 11 dB
	VNADLL_API Attenuation ATTEN_12;         //!< Attenuation value of 12 dB
	VNADLL_API Attenuation ATTEN_13;         //!< Attenuation value of 13 dB
	VNADLL_API Attenuation ATTEN_14;         //!< Attenuation value of 14 dB
	VNADLL_API Attenuation ATTEN_15;         //!< Attenuation value of 15 dB
	VNADLL_API Attenuation ATTEN_16;         //!< Attenuation value of 16 dB
	VNADLL_API Attenuation ATTEN_17;         //!< Attenuation value of 17 dB
	VNADLL_API Attenuation ATTEN_18;         //!< Attenuation value of 18 dB
	VNADLL_API Attenuation ATTEN_19;         //!< Attenuation value of 19 dB
	VNADLL_API Attenuation ATTEN_20;         //!< Attenuation value of 20 dB
	VNADLL_API Attenuation ATTEN_21;         //!< Attenuation value of 21 dB
	VNADLL_API Attenuation ATTEN_22;         //!< Attenuation value of 22 dB
	VNADLL_API Attenuation ATTEN_23;         //!< Attenuation value of 23 dB
	VNADLL_API Attenuation ATTEN_24;         //!< Attenuation value of 24 dB
	VNADLL_API Attenuation ATTEN_25;         //!< Attenuation value of 25 dB
	VNADLL_API Attenuation ATTEN_26;         //!< Attenuation value of 26 dB
	VNADLL_API Attenuation ATTEN_27;         //!< Attenuation value of 27 dB
	VNADLL_API Attenuation ATTEN_28;         //!< Attenuation value of 28 dB
	VNADLL_API Attenuation ATTEN_29;         //!< Attenuation value of 29 dB
	VNADLL_API Attenuation ATTEN_30;         //!< Attenuation value of 30 dB
	VNADLL_API Attenuation ATTEN_31;         //!< Attenuation value of 31 dB
	/** @}*/


	typedef int RFPath;
	// These are the paths available on the switchboard used by the VNA product
	VNADLL_API RFPath PATH_T1R1; //!< Out port T1, in port T1
	VNADLL_API RFPath PATH_T1R2; //!< Out port T1, in port T2
	VNADLL_API RFPath PATH_T2R1; //!< Out port T2, in port T1
	VNADLL_API RFPath PATH_T2R2; //!< Out port T2, in port T2
	VNADLL_API RFPath PATH_REF;  //!< Reference path
	...
	VNADLL_API SParameter PARAM_S11;  //!< S11 Parameter
	VNADLL_API SParameter PARAM_S21;  //!< S21 Parameter
	VNADLL_API SParameter PARAM_S12;  //!< S12 Parameter
	VNADLL_API SParameter PARAM_S22;  //!< S22 Parameter
	/** @}*/
	...
	VNADLL_API CalibrationStep STEP_P1_OPEN;   //!< Calibration step for measuring a open on port 1
	VNADLL_API CalibrationStep STEP_P1_SHORT;  //!< Calibration step for measuring a short on port 1
	VNADLL_API CalibrationStep STEP_P1_LOAD;   //!< Calibration step for measuring a load on port 1
	VNADLL_API CalibrationStep STEP_P2_OPEN;   //!< Calibration step for measuring a open on port 2
	VNADLL_API CalibrationStep STEP_P2_SHORT;  //!< Calibration step for measuring a short on port 2
	VNADLL_API CalibrationStep STEP_P2_LOAD;   //!< Calibration step for measuring a load on port 2
	VNADLL_API CalibrationStep STEP_THRU;      //!< Calibration step for measuring a through from port 1 - port 2
	/** @}*/
	 ...
	 */
	VNADLL_API ErrCode measureUncalibrated(TaskHandle t,
	 ...
	 *       - ERR_INTERRUPTED if the measurement was interrupted
	 */
	VNADLL_API ErrCode measure2PortCalibrated(TaskHandle t,
...
// for each calibration step
	 ...
	 */
	VNADLL_API size_t getCalibrationNumberOfFrequencies(TaskHandle t);
