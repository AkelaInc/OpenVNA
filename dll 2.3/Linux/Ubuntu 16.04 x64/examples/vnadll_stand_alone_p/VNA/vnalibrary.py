...
ERR_MISSING_ATTEN       = ErrCode.in_dll(dll, "ERR_MISSING_ATTEN"      ).value
ERR_NO_ATTEN_PRESENT    = ErrCode.in_dll(dll, "ERR_NO_ATTEN_PRESENT"   ).value
ERR_BAD_PORT            = ErrCode.in_dll(dll, "ERR_BAD_PORT"           ).value
...
Exception_Map = {
	...
	ERR_MISSING_ATTEN      : vnaexceptions.VNA_Exception_Missing_Attenuator_Setting,
	ERR_NO_ATTEN_PRESENT   : vnaexceptions.VNA_Exception_No_Attenuator_Present,
	ERR_BAD_PORT           : vnaexceptions.VNA_Exception_Bad_Port,
...
ErrCodeBOOK = 	{
					...
					ERR_MISSING_ATTEN      : 'ERR_MISSING_ATTEN',
					ERR_NO_ATTEN_PRESENT   : 'ERR_NO_ATTEN_PRESENT',
...
class RAW_VNA(object):
	...
	def start(self):
		...
		self.handleReturnCode(ret, message="Current state = '%s'" % state)
	...
	def setFrequencies(self, freqs):
		...
		is an array of desired frequencies. Note that the VNA frequency generation hardware has fixed
		...
		Args:
			freqs - An iterable which returns a set of frequencies to measure.
				 The frequency points *to not* have to be linear,
				...
				sweep times.
				The band-change boundaries can be found using getHardwareDetails().
		...
		# Bake up the ctypes array.
		N = len(freqs)
		dafreq = DoubleArrayFactory(N)(*freqs)

		tmp = dll.setFrequencies
		tmp.argtypes = [TaskHandle, DoubleArrayFactory(N), ct.c_uint]
		tmp.restype = ErrCode
		ret = tmp(self.__task, dafreq, N)
		self.handleReturnCode(ret)
	...
	def getStateReadable(self):
		''' Get the current state of the Task object as a human-readable string.

		Args:
			None

		Returns:
			Returns an ascii-string representation of the current task state
		'''
		return TaskStateBOOK[self.getState()]
	...
	def getFrequencies(self):
		...
		Returns:
				If no frequencies have been set, this function returns an empty numpy array.
				If sweep frequencies have been set (via either setFrequencies() or utilGenerateLinearSweep()),
				this function returns a 1D numpy array containing the actual frequencies that will be
				synthesized during the sweep.
	...
	def utilNearestLegalFreq(self, target_freq):
		...
		VNA hardware. This is not available in the TASK_UNINITIALIZED state (it requires the hardware
		information which is fetched during initialization).
	...
	def utilFixLinearSweepLimits(self, target_start_freq, target_end_freq, N):
		...
		This may move the start and end frequencies of your scan slightly (<1 MHz). Generally,
		taking many tightly-spaced points results in larger end-point movements, due to the smaller
		solution space resulting from the more limited sweep span.
	...
	def handleReturnCode(self, code, message = ""):
		...
		if code in Exception_Map:
			state = TaskStateBOOK[self.getState()]
			raise Exception_Map[code](message + " Current state: {}".format(state))
		if code != ERR_OK:
			raise vnaexceptions.VNA_Exception("Unknown return code from call! Code: %s" % code)
